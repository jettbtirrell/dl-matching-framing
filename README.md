...
2. **Assignment briefs are serialized** by joining all filled form fields: topic, key takeaway, context, target audience, values, tone. Empty optional fields are excluded.

3. **Creator embeddings are pre-computed** and stored in `data/creator-embeddings.json` (generated by `npm run generate-embeddings`). On server startup, the file is loaded into a module-level `Map<uniqueId, vector>` — no API call for creators. Each entry stores an `embeddedAt` timestamp; any creator whose `updatedAt` is newer gets automatically re-embedded in a targeted batch call and the file is updated. If the file is missing (first setup), all creators are embedded in a single API call (~150ms) and the in-memory cache is used for subsequent requests.

4. **Per request:** the assignment brief is embedded in one API call (~100ms). This is the only per-request network cost for scoring in normal operation.
...
**3. Per-field embeddings with tunable weights.**
Instead of one holistic vector per creator, embed each dimension separately (niches, values, tone, causes) and combine similarity scores with configurable weights. This lets you say "niche match matters 2× as much as tone match" and then validate that claim against actual selection data. Currently the model treats all fields as equally important — which is probably wrong, but we have no data yet to know the right weights.

**4. Structured output mode for the LLM.**
